...
conversion, explicit connector arguments, one connection/cursor lifecycle per
request, and the Snowglobe-owned recursive AST policy. Incremental Snowflake Arrow
retrieval and real-executor cursor registration remain planned work. Snowglobe's
model-facing implementation is its own low-level MCP surface as recorded in
[ADR 0002](decisions/0002-low-level-mcp.md); no Querido MCP code is reused.
